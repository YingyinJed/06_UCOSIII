...
#define SRAMBANK 	3	//定义支持的SRAM块数.	探索版内部内存有3块所以设置为3

//计算公式
//MEM_ALLOC_TABLE_SIZE=MEM_MAX_SIZE÷MEM_BLOCK_SIZE
//管理MEM_MAX_SIZE大小的内存，总共所需内存大小为：
//MEM_MAX_SIZE+MEM_ALLOC_TABLE_SIZE*2（字节）


//mem1内存参数设定.mem1完全处于内部SRAM里面。STM32F407的内部内存池128K，现管理100K用了106.25KB内存
#define MEM1_BLOCK_SIZE			32  	  						//内存块大小为32字节
#define MEM1_MAX_SIZE			100*1024  						//最大管理内存 100K
#define MEM1_ALLOC_TABLE_SIZE	MEM1_MAX_SIZE/MEM1_BLOCK_SIZE 	//内存表大小

//mem2内存参数设定.mem2的内存池处于外部SRAM里面.外扩内存池1024K，现管理1024K用了1020K
#define MEM2_BLOCK_SIZE			32  	  						//内存块大小为32字节
#define MEM2_MAX_SIZE			960 *1024  						//最大管理内存960K
#define MEM2_ALLOC_TABLE_SIZE	MEM2_MAX_SIZE/MEM2_BLOCK_SIZE 	//内存表大小61440
...
struct _m_mallco_dev
{
	...
	uint16_t *memmap[SRAMBANK]; 				//内存管理状态表，由于这是个u16的指针最大值为65535，假设每个内存块为32字节，那么最大可申请内存就是2M内存
																																											//因此如果你的内存块大小为64那么就可以生成4M内存了
